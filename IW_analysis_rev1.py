...
def adiabatic_level(params=default_params):
    """
...
def strainFromCtd(params=default_params):
    """
    Calculate Strain Using Adiabatic Leveling Method
    """

    # Get Reference N2 from Adiabatic Leveling
    N2Ref, PS, PT, z_ctd, lat, lon = adiabatic_level(params=default_params)
...
def analyzeShearStrainRw(ctd_dict, ladcp_dict, params=default_params):
    ...
    Strain, N2, N2b, rho = strainFromCtd()
...
def transectPlots(params=default_params):
    ...
    Rw, ShearInt, StrainInt, ShearSpec, StrainSpec, StrainPow, ShearPow, N2,\
                mxShear, kxShear, mxStrain, kxStrain, Rho, shear, zbin\
                = analyzeShearStrainRw(ctd_dict, ladcp_dict)

    yaxes = [0, 4000]
    xlabels = 'Distance Travelled (km)'
    ylabels = 'Depth (m)'
    # CTD plots
    ...
    c1 = plt.contourf(distGrid, depth_ctd, PT,\
                 cmap=cmocean.cm.thermal)
    plt.colorbar(c1, label='Temperature ($^\circ$C)')
    plt.contour(distGrid, depth_ctd, PT, levels=levels,\
                colors='k', linewidths=.25)
    for i, distIn in enumerate(dist):
        plt.annotate(str(i+1), (distIn, 200))
        plt.scatter(distGrid[0:-1:50,i], depth_ctd[0:-1:50,i], s=.5, color='k', alpha=.5)
    plt.ylim(yaxes)
    plt.gca().invert_yaxis()
...
#                colors='k', linewidths=.25)
    ...
    # Pdens
    levels = np.arange(np.nanmin(sigma0), np.nanmax(sigma0), 0.01)
    fig1 = plt.figure(figsize=figSizeTrans)
    c = plt.contourf(distGrid, depth_ctd, sigma0,\
...
#    
    ...
    rcf = plt.contourf(dist, zplots, Rw, cmap=cmocean.cm.curl)
    plt.colorbar(rcf, label='Rw')
    for i, distIn in enumerate(dist):
        plt.annotate(str(i+1), (distIn, 200))
        plt.scatter(distGrid[0:-1:50,i], depth_ctd[0:-1:50,i], s=.5, color='k', alpha=.5)
    plt.ylim(yaxes)
    plt.gca().invert_yaxis()
    plt.fill_between(dist, bathy, 4000, color = '#B4B4B4')
    plt.title('Rw - Shear (N2 normalized) to Strain Ratio')
    plt.xlabel('Distance along Transect')
    plt.ylabel('Pressure (dB)')
    plt.tight_layout()
    
    plt.savefig('figures/Rw.png', bbox_inches='tight', dpi=400)

...
#        U2, V2, p_ladcp, Uspec, Vspec, etaSpec = iwp.frequencyEstimator(ctd, ladcp, bathy,\
...
def lee_wave_tests(kh, omega, N2, ctd, ladcp, dist, depths, error_factor=2, plots=False):
    ...
    for i, dump in enumerate(kh.T):
        ...
        testA = np.abs(dshift[:,i]**2-f**2) <= 2*f**2
...
def energy_flux(k, l, N2, ladcp, ctd, m=300):
    """
    ...
    # f grid
    f = gsw.f(lat)
    f = np.tile(f.T,E.shape[0]).T
    a = aspect
    
    # Vertical Group Speed - Use the same one as in ray tracing 
#    CGz = -((omega**2 - f**2)/(omega*(k**2+l**2+m**2)))*m**2
    CGz = -(a**2)*(N2-f**2)/(m*((1+a**2)**1.5)*np.sqrt(f**2 + a*N2))
